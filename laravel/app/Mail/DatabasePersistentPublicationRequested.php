...
class DatabasePersistentPublicationRequested extends Mailable
{
    ...
    {
        return new Envelope(
            subject: config('app.name') . ': ' . ($this->admin ? '(ADMIN): ' : '') . ' Persistent publication requested for the database "' . $this->database->title . '"' . ($this->admin ? ' (' . $this->database->id . ')' : '') . ' - awaiting approval',
        );
    }
